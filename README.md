...
- **Weekly analyze** — `scripts/weekly_analyze.py` content-type-classifies (Haiku) then aspect-tags (Qwen-local) only the new mentions, then writes a per-week aggregate **snapshot** (pure-Python aggregation; no brief synthesis). Content-type classification is a required first step — mentions without a content-type tag are dropped by the aspect-tagging gate, so a newly onboarded product would otherwise show no data — and is idempotent. The sequence of snapshots is the **Trend over time** view (`/trend`). *(Shipped sessions 46–47; content-type wired into the weekly pass session 50.)*
