...
- **`tests/unit/scripts/`** — CLI-script pure helpers (scripts are loaded via `importlib`, not on the package path). `test_add_product.py` (session 48): slugify, name→`primary` regex derivation, URL→`secondary` SKU-pattern extraction, `build_product` id/brand inference + overrides, `render_block` YAML shape, and `append_to_yaml` round-trip + revert-on-invalid-reload. The interactive prompt, network fetch, and the DB-backed `preview_matches` are not unit-tested (I/O); `preview_matches` is exercised by operator runs against the live corpus.
- **`tests/unit/scheduling/`** — cadence state files + weekly snapshot identity. `test_state.py` (quarterly refresh: read/write/atomic, `is_due`, `days_since_refresh`). `test_daily_state.py` (session 45 daily collector: schema-versioned atomic JSON round-trip + gap detection — never-run is a cold start not a gap, healthy 24h cadence has no gap, missed day past the 36h threshold flags). `test_weekly_snapshot.py` (session 46: `snapshot_run_id` ISO-week format + zero-padding + within-week stability + ISO-year boundary; `ensure_run_row` inserts when absent / no-ops without overwrite when present). State files fail loud on schema-version mismatch and only advance state on success.
...
- `src/lib/pairExportMarkdown.test.ts` — 12 tests on the pure `pairBriefToMarkdown` + helpers `computePairLeaders` / `buildPairCitedIds`: filters ties and rows below the `_PAIR_LEAD_THRESHOLD` (0.10); sorts leaders by `abs(delta)` desc and caps at 3 per side; cites up to 2 mention_ids from the leading side per bullet; dedups reading-order union of contrast cites → primary leader cites → competitor leader cites; renders H1 "{primary} vs {competitor}"; 3-col snapshot (leads / coverage / avg-net per side) with ties footnote; 11-row aspect comparison table with em-dashes for missing cells and `tie` label when neither side leads; contrast paragraph with `[n, m]` refs; leader bullets in operator-preview format (`net X vs Y (Δ +Z) [n, m]`); empty-state fallback line when one side has no leaders above threshold; omits the Citations section when `citedIds` is empty; renders Citations footer with source / @author / date.
...
- `src/components/atoms/VerbatimCard.test.tsx` — 4 tests on the aspect-scoped chip (session 44): a multi-aspect mention with `focusAspect` shows the focus aspect's tag (not the arbitrary first tag); falls back to the first tag when no focus is given (pooled context); disambiguates by `focusProductId` when a mention tags the same aspect for two products; omits the chip rather than mislabel when the focus aspect has no tag.
- `src/components/EvidenceDrawer.test.tsx` — 2 tests on the aspect-scoped drill (session 44): each card's chip renders the drilled aspect (header aspect matches chip aspect, not the first tag); the intensity filter is scoped to the drilled aspect (a high-on-another-aspect mention is dropped by a high-on-this-aspect filter).
