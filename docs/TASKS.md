...
**Active:** Polish ordering locked session 36 — 3 buckets (briefs · reliability · cosmetic) sequenced BEFORE 3 exit forks. **Polish queue 9/9 shipped through session 39** (bite #1 Notebookcheck manufacturer POST · bite #2 MSI regex refinement · bite #3 F1 closure via loose-metric · bite #4 Stage B integration test · bite #5 link-rot/tombstoning [live verification deferred] · bite #6 quarterly-cadence scheduler skeleton · bite #7 visual loading skeletons · bite #8 design-system audit reconciliation · bite #9 markdown brief export). **Next: bite-#5 follow-on (reddit-domain throttling)** — recommended path: per-domain semaphore + 1.5 s/req sleep loop for `*.reddit.com` URLs as a 1-hr bite; fall back to authenticated reddit API only if anonymous single-threaded still gets rate-limited. Exit forks (unlock after bite-#5 follow-on): operator brief-review walk · reproducibility re-run · stakeholder prep packet. **674/674 unit tests · 3/3 integration tests · mypy clean (137 src) · ruff clean · frontend tsc + vite build green.** Current frontend bundle: `index-CrE6PHQu.js` (294.07 KB / 87.01 KB gzipped; replaces session-38 `index-2cxKn-Gl.js`; new hash breaks stale browser cache). Migration head: `eb05da255474` (unchanged; no schema work session 39).
...
- [x] Loading skeletons (shadcn defaults) — `frontend/src/components/ui/skeleton.tsx` (vanilla `animate-pulse` + `bg-muted` + a11y attrs `role="status"` / `aria-busy` / `aria-live`) shipped session 37 across 7 spots (Compare heatmap · Standalone picker / full-page / brief panel · Pair scorecard · EvidenceDrawer · CitationPanel)
...
- [x] Tombstoning (`scripts/verify_mention_links.py` + `pulse_check/verifiers/link_verifier.py` + aggregation filter + `MentionView` API + VerbatimCard "link dead" chip + 15 unit + 1 integration test; live HEAD verification deferred behind reddit-domain throttling — bite-#5 follow-on)
