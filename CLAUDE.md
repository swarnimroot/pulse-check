...
5. **`docs/TASKS.md` tracks wave-level deliverables (the forward plan); `docs/SESSION_LOG.md` chronicles bites (the historical record).** TASKS.md stays flat and scannable — each wave is a checklist of high-level deliverables with no sub-bite numbering, no `(session N)` breadcrumbs, no narrative. Those live in SESSION_LOG entries. TASKS.md is updated immediately on any state change — triggers: (a) operator approval that closes a deliverable, (b) plan deviation that adds / removes / re-scopes a deliverable, (c) wave boundary cross, (d) the top "Status" / "Active" / "Last reconciled" lines whenever they go stale. Session close is a recap of already-reconciled state, not the place where reconciliation happens. The next-session audit checklist diffs `[x]` set in TASKS.md against the prior session's claimed deliverable closes (read from SESSION_LOG); mismatches are flagged before any forward work.
